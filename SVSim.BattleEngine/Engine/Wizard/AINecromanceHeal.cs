...
public class AINecromanceHeal : AIFiltersAndSelectTypeArgument
{
	private AIPolishConvertedExpression _healAmout;

	protected override int SELECT_TYPE_OFFSET => 2;

	protected override int NON_FILTER_FIRST_OFFSET => SELECT_TYPE_OFFSET;

	public AINecromanceHeal(string text)
		: base(text)
	{
	}
	...
	protected override void InitExpressions(string text)
	{
		...
		_healAmout = _exprList[_exprList.Count - 1];
	}
	...
	private int GetHealAmount(AIVirtualCard tagOwner, AIVirtualField field, List<int> playPtn, AISituationInfo situation)
	{
		if (_healAmout == null)
		{
			AIConsoleUtility.LogError("AINecromanceHeal.GetHealAmount() error!! _healAmount is null");
			return 0;
		}
		return (int)_healAmout.EvalArg(tagOwner, playPtn, field, situation);
	}
}
