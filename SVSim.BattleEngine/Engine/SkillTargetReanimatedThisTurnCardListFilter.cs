...
public class SkillTargetReanimatedThisTurnCardListFilter : ISkillTargetFilter
{
	public IEnumerable<IReadOnlyBattleCardInfo> Filtering(IEnumerable<IBattlePlayerReadOnlyInfo> battlePlayerInfos, SkillConditionCheckerOption option)
	{
		...
		list.AddRange(battlePlayerInfos.SelectMany((IBattlePlayerReadOnlyInfo p) => from t in p.SkillInfoGameReanimatedCards
			where t.Turn == turn && t.IsSelfTurn == isSelfTurn
			select t.Card));
		return list;
	}
}
