...
public class SkillTargetReturnThisTurnCardListFilter : ISkillTargetFilter
{
	public IEnumerable<IReadOnlyBattleCardInfo> Filtering(IEnumerable<IBattlePlayerReadOnlyInfo> battlePlayerInfos, SkillConditionCheckerOption option)
	{
		...
		list.AddRange(battlePlayerInfos.SelectMany((IBattlePlayerReadOnlyInfo p) => from pp in p.SkillInfoReturnedCards
			where pp.Turn == turn && pp.IsSelfTurn == isSelfTurn
			select pp.Card));
		return list;
	}
}
